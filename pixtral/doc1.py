from transformers import pipeline, AutoModelForCausalLM, AutoTokenizer, AwqConfig
import torch

model_id = "mrm8488/bertin-gpt-j-6B-ES-8bit"
...
chunks = split_text(jud_text, chunk_size=400)  # Split the text into chunks of 400 tokens

#Forma chatGPT
summaries = [summarizer(chunk, max_new_tokens=50, min_new_tokens=50, length_penalty=2.0, num_beams=4, early_stopping=True)[0]['summary_text'] for chunk in chunks]
...
#     summ = summarizer(chunks[i], max_new_tokens=300, min_new_tokens=250, num_beams=4)[0]['summary_text']
#     iteration_summ = " ".join([summ, chunks[i+1]])

# final_summary = summarizer(iteration_summ, max_new_tokens=500, min_new_tokens=400, length_penalty=2.0, num_beams=4, early_stopping=True)[0]['summary_text']
